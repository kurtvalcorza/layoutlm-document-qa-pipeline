...
from layoutlm_document_qa_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"model_type": "layoutlm", "max_2d_position_embeddings": 1024}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        LayoutLMDocumentQAPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
