...
- Pins: `torch==2.14.0`, `transformers==4.57.6`, `safetensors==0.8.0`, `numpy==2.5.3`, `pillow==11.3.0`, `huggingface-hub==0.36.2`; Python 3.12. `pytesseract` and Tesseract are **not** pinned or installed.
...
- Tutorial execution: `tutorials/layoutlm_document_qa_colab.ipynb` ran top-to-bottom in a fresh local kernel; recorded in `docs/release-verification.md` as pre-flight, not supported-runtime evidence.
